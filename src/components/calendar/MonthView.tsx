...
export function MonthView({
  ...
  return (
    <div className="glass-panel-3d rounded-2xl border border-slate-800 shadow-2xl overflow-hidden">
      ...
      <div className="p-5 border-b border-slate-800 flex items-center justify-between bg-slate-900/60">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="px-3 py-1.5 rounded-lg border border-slate-700 bg-slate-800 text-xs font-semibold text-slate-200 hover:bg-slate-700 transition-colors shadow-sm"
          >
            Today
          </button>
      ...
      {/* Days Grid */}
      <div className="grid grid-cols-7 auto-rows-fr divide-x divide-y divide-slate-800 min-h-[580px] bg-slate-950/40">
        {days.map((day) => {
          ...
          return (
            <div
              ...
              className={`p-2 min-h-[100px] flex flex-col justify-between group transition-colors ${
                isCurrentMonth ? 'bg-slate-900/40' : 'bg-slate-950/80 text-slate-600'
              } ${isCurrentDay ? 'bg-blue-950/30' : ''}`}
            >
              {/* Day Number Header */}
              <div className="flex items-center justify-between mb-1">
                <span
                  className={`text-xs font-semibold w-6 h-6 rounded-full flex items-center justify-center ${
                    isCurrentDay
                      ? 'bg-[#0a66c2] text-white shadow-md shadow-blue-500/40 font-bold'
                      : isCurrentMonth
                      ? 'text-slate-300'
                      : 'text-slate-600'
                  }`}
                ...
                <button
                  ...
                  className="opacity-0 group-hover:opacity-100 p-1 text-slate-400 hover:text-cyan-400 hover:bg-slate-800 rounded transition-opacity"
              ...
              <div className="space-y-1 overflow-y-auto max-h-[85px] scrollbar-none flex-1">
                {dayPosts.map((post) => (
                  <button
                    ...
                    className={`w-full text-left p-1.5 rounded-lg border text-[11px] font-medium truncate flex items-center gap-1.5 shadow-sm hover:scale-[1.02] transition-transform ${getStatusColor(
                      post.status
                    )}`}
                  >
                    {post.scheduledAt && (
